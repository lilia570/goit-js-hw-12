...
btnLoad.addEventListener('click', async () => {
  ...
  try {
    ...
    if (responseData.hits.length === 0 || currentPage >= totalPages) {
      iziToast.info({
        title: "We're sorry, but you've reached the end of search results.",
        position: 'topRight',
      });
  ...
  showLoadMoreButton();
    ...
    const firstItem = document.querySelector('.image-card'); 
    if (firstItem) {
      const heightOneItem = firstItem.getBoundingClientRect().height;
      window.scrollBy({
        top: heightOneItem * 2,
        behavior: 'smooth',
      });
